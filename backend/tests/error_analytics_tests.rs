...
async fn test_build_error_analytics_empty() {
    let pool = PgPool::connect("postgres://postgres:postgres@localhost/crucible_test").await.unwrap();
    let redis = RedisClient::open("redis://127.0.0.1/").unwrap();
    let app = error_analytics_routes(pool.clone(), redis.clone());

    // Ensure DB is empty
    sqlx::query("DELETE FROM build_errors").execute(&pool).await.unwrap();
...
async fn test_build_error_analytics_with_data() {
    ...
    // Insert test data
    sqlx::query("DELETE FROM build_errors").execute(&pool).await.unwrap();
    sqlx::query("INSERT INTO build_errors (error_type, message, occurred_at) VALUES ($1, $2, NOW())")
        .bind("TypeA").bind("Error A").execute(&pool).await.unwrap();
    sqlx::query("INSERT INTO build_errors (error_type, message, occurred_at) VALUES ($1, $2, NOW())")
        .bind("TypeB").bind("Error B").execute(&pool).await.unwrap();
